...
trait Rules {
  ...
  /**
   * Returns an algorithm that executes repeatedly a randomly chosen enabled rule amongst the
   * declared rules, forever, or until no rule applies.
   *
   * @return the algorithm.
   */

  protected def Random : Alg = AlgRepeatRandom(storedRules)

  /**
   * Returns an algorithm that executes repeatedly a randomly chosen enabled rule
   * amongst the declared rules, max` times, or until no rule applies.
   *
   * @param max the number of times a rule should be picked and executed.
   * @return the algorithm.
   **/

  protected def Random(max: Int) : Alg = AlgRepeatRandomBounded(max, storedRules)
  ...
  protected def Random(rules: Rule*) : Alg = AlgRepeatRandom(rules.toList)
  ...
  protected def Random(max: Int, rules: Rule*) : Alg = AlgRepeatRandomBounded(max, rules.toList)
  ...
  protected def All(rules: Rule*) : Alg = AlgSeqAll(rules.toList)
  ...
  protected def Enabled(rules: Rule*) : Alg = AlgSeqEnabled(rules.toList)
  ...
  protected def First(rules: Rule*) : Alg = AlgSeqFirst(rules.toList)
  ...
  protected def Until(rules: Rule*) : Alg = AlgSeqUntil(rules.toList)
